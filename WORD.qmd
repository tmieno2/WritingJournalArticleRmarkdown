...
If you are trying to decide whether to use the rmd-WORD system, you should be aware that it is very likely that you cannot just do everything in rmarkdown unlike wirting to PDF. Specifically, you will  almost certainly need to format your manuscript manually in WORD itself to satisfy the format requirements by the final desination of your manuscript. This is because rmarkdown provides very weak formatting capabilities. 

Here are the proposed steps to generate a fully publication-ready article: 

1. Write an Rmarkdown file and knit to a WORD file (the focus of this lecture) without worrying about formatting your manuscript (Most jorunals do not require manuscript formatting until the manuscript is accepted. But, please do check yourself it is indeed the case)
2. Submit your masnucript
3. Revise and resubmit your manuscript 
4. Repeat steps 2 and 3 until accepted (terminate the process if rejeccted)
5. Format (partially through Rmarkdown, but mostly in WORD) your manuscript to the specific requirements by the journal

Note that this process is recommended when you are knitting to PDF as well. It is just that it is more important to stick with it if you are using rmd-WORD system. 
